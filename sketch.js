...
function preload() {
  ...
  for (let i = 0; i < stageCount; i++) {
    const imagePath = `materials/images/Stg_${i + 1}D.png`;
    try {
...
function setup() {
  ...
  for (let i = 0; i < stageCount; i++) {
    ...
    stages.push({
      ...
      interactive: i < 4 // Set interactivity to true for the first four nodes, false for others
    });
  }
    ...
    for (let i = 0; i < stages.length; i++) {
      ...
      if (distance < nodeRadius) {
        if (stages[i].interactive) {
          ...
          // Play locked node sound for inactive nodes
...
// Example: To turn off interactivity for all nodes except the first four
